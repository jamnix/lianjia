...
if __name__=="__main__":
	...
	for line in file_fd:
		if line.find(last_str) != -1 and line.find("heartbeat") != -1:
			...
			path = "%s/data/%s_web_lf.rrd"%(config.HOME, name)	
			output="%s/html/mrtg/%s_web_lf.png"%( config.HOME, name )
			output_weekly="%s/html/mrtg/weekly/%s_web_lf.png"%( config.HOME, name )
			if os.access(path, os.F_OK) is False:
				create_rrd(path, str(ts-1))
		
			lf = int(params['lf'][0])
			update_rrd(path, lf, ts);
